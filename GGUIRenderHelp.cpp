...
{
	const char* szText = pUIText->GetText().c_str();
	if (szText == 0 || szText[0] == 0)
	{
		return;
	}
	const GGUIImageset* pImageset = GGUIImagesetManager::Get()->GetImagesetByName(pUIText->GetFont());
	if (pImageset == 0)
	{
		return;
	}
	...
	const GGUIColor& kColor = pUIText->GetColor();
	//
